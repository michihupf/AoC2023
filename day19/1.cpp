...
class Part {
  public:
    ...
    int      &operator[](char variable_name) { return m_variables[variable_name]; }
    const int operator[](char variable_name) const {
        return m_variables.find(variable_name)->second;
    }

    int sum() const {
        return std::accumulate(m_variables.begin(), m_variables.end(), 0, [](const int prev, const auto &p) {
...
struct Condition {
    ...
    inline bool operator()(int variable) const {
...
struct Workflow {
    ...
    const std::string &run_on(const Part &part) {
